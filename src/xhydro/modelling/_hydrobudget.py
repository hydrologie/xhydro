...
class Hydrobudget(HydrologicalModel):
    ...
    def _standardise_outputs(self, **kwargs):
        ...
        qflow_sim_an["frequence"]=frequence_list
        ...
        qflow_sim_unique["frequence"]=np.concatenate((frequence_list,frequence_list), axis=None)  
        ...
        qflow_obs_an["frequence"]=frequence_list
        ...
        qflow_obs_unique["frequence"]=np.concatenate((frequence_list,frequence_list), axis=None)  
                
        # application des poids aux mesures pour privilégier les bilans annuels puis saisonniers puis mensuels 
        if frequency=="all":
            qflow_sim_unique[qflow_sim_unique["frequence"]=="year"]["q"]= qflow_sim_unique[qflow_sim_unique["frequence"]=="year"]["q"]*5
            qflow_sim_unique[qflow_sim_unique["frequence"]=="season"]["q"]= qflow_sim_unique[qflow_sim_unique["frequence"]=="year"]["q"]*3.3
            qflow_sim_unique[qflow_sim_unique["frequence"]=="month"]["q"]= qflow_sim_unique[qflow_sim_unique["frequence"]=="year"]["q"]*1.7
            qflow_obs_unique[qflow_sim_unique["frequence"]=="year"]["q"]= qflow_sim_unique[qflow_sim_unique["frequence"]=="year"]["q"]*5
            qflow_obs_unique[qflow_sim_unique["frequence"]=="season"]["q"]= qflow_sim_unique[qflow_sim_unique["frequence"]=="year"]["q"]*3.3
            qflow_obs_unique[qflow_sim_unique["frequence"]=="month"]["q"]= qflow_sim_unique[qflow_sim_unique["frequence"]=="year"]["q"]*1.7
        ...
        #     if qflow_obs_unique.loc[a,'frequence']=="year":
        #         year_unique=list(set(qflow_obs_unique[qflow_obs_unique.loc[a,'frequence']=="year"]["temps_list"]))
        #         times.append(datetime.strptime(str(year_unique[a]), "%Y"))
        #     if qflow_obs_unique.loc[a,'frequence']=="season":
        ...
        #     if qflow_obs_unique.loc[a,'frequence']=="month":
        #         year_unique=qflow_obs_unique.loc[a,"temps_list"].split("_")[1]
        #         trim_unique=qflow_obs_unique.loc[a,"temps_list"].split("_")[0] 
        #         times.append(datetime.strptime(year_unique, "%Y") + timedelta(days=int(trim_unique)*30.42))
        
        if frequency=="all":
            subsety_year=qflow_obs_unique[qflow_obs_unique['frequence']=='year']
            subsety_season=qflow_obs_unique[qflow_obs_unique['frequence']=='season']
            subsety_month=qflow_obs_unique[qflow_obs_unique['frequence']=='month']
            times=[] 
    ...
    def plot_streamflow_scatter_color(
        ...
        while decompte<fin_decompte:
            ...
            fig_path1=Path(self.output_dir,f"SP_{freq[decompte]}_tot_allstations_{self.timestamp}.png")
            ...
            legend = ax.legend(colors, numpoints=1, fontsize=10,
                               ...
                               bbox_to_anchor=(anchor_y, 0), ncol=1)
            legend.draw_frame(False)
            fig_path2=Path(self.output_dir,f"SP_{freq[decompte]}_base_allstations_{self.timestamp}.png")
