...
class GetFlow_ECCC(GetFlow):
    ...
    def decode_file(self, filepath):
        ...
        df = pd.read_csv(filepath, comment="#", header=0, compression=self.compression, dtype={"Value": "float", "Grade": "int", "Qualifiers": "str"})
...
class GetFlow_CEHQ(GetFlow):
    ...
    def obtain_file_list(self):
        ...
        print("Found these files on CEHQ server:")
    ...
    def decode_file(self, filepath):
        ...
        try:
            df = pd.read_fwf(StringIO(table_str), sep=r"\s{2,}", engine="python", dtype={"Station": str}, names=columns, colspecs=colspecs)
            df.pop("Station")
        except Exception as e:
