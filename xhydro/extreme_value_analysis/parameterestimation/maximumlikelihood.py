...
def gevfit_3(df: pd.DataFrame, datacol: str, locationcovid: list[str] = [], logscalecovid: list[str] = [], shapecovid: list[str] = []) -> MaximumLikelihoodAbstractExtremeValueModel:
...
def gevfit_4(df: pd.DataFrame, datacol: str, initialvalues: list[float], locationcovid: list[str] = [], logscalecovid: list[str] = [], shapecovid: list[str] = []) -> MaximumLikelihoodAbstractExtremeValueModel:
    jl_df = pd_dataframe_to_jl_dataframe(df) 
    jl_datacol = py_str_to_jl_symbol(datacol)
...
def gumbelfit_3(df:pd.DataFrame, datacol: str, locationcovid: list[str] = [], logscalecovid: list[str] = []) -> MaximumLikelihoodAbstractExtremeValueModel:
    jl_df = pd_dataframe_to_jl_dataframe(df) 
    jl_datacol = py_str_to_jl_symbol(datacol)
...
def gumbelfit_4(df:pd.DataFrame, datacol: str, initialvalues: list[float], locationcovid: list[str] = [], logscalecovid: list[str] = []) -> MaximumLikelihoodAbstractExtremeValueModel:
    jl_df = pd_dataframe_to_jl_dataframe(df) 
    jl_datacol = py_str_to_jl_symbol(datacol)
    jl_initialvalues = py_list_to_jl_vector(initialvalues)
    jl_locationcovid, jl_logscalecovid= jl_symbol_fit_parameters([locationcovid, logscalecovid])
...
def gpfit_3(df: pd.DataFrame, datacol: str, logscalecovid: list[str] = [], shapecovid: list[str] = []) -> MaximumLikelihoodAbstractExtremeValueModel:
    jl_df = pd_dataframe_to_jl_dataframe(df) 
    jl_datacol = py_str_to_jl_symbol(datacol)
...
def gpfit_4(df: pd.DataFrame, datacol: str, initialvalues: list[float], logscalecovid: list[str] = [], shapecovid: list[str] = []) -> MaximumLikelihoodAbstractExtremeValueModel:
    jl_df = pd_dataframe_to_jl_dataframe(df) 
    jl_datacol = py_str_to_jl_symbol(datacol)
    jl_initialvalues = py_list_to_jl_vector(initialvalues)
    jl_logscalecovid, jl_shapecovid = jl_symbol_fit_parameters([logscalecovid, shapecovid])
