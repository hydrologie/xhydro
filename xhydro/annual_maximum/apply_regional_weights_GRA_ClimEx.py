...
# Load data
mat_obs_data = scipy.io.loadmat(os.path.join(data_folder, f'INFO_Crue_obs_stations_{simulation_type}_ClimEx_{obs_stations_to_select}.mat'))
obs_data = mat_obs_data['all_Qsim']
mat_gra_weights = scipy.io.loadmat(os.path.join(results_folder, f'GRA_{simulation_type}_{obs_stations_to_select}_weights.mat'))
regional_weights = mat_gra_weights['regional_weights']
mat_gra_data = scipy.io.loadmat(os.path.join(save_folder, f'GRA_{simulation_type}_{obs_stations_to_select}_ClimEx.mat'))
all_data = mat_gra_data['all_data']
...
save_path = os.path.join(save_folder, f'GRA_{simulation_type}_{obs_stations_to_select}_ClimEx.mat')
scipy.io.savemat(save_path, {'regional_weights': regional_weights, 'all_data': all_data}, format='5', long_field_names=True)
