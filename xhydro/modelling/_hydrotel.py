...
class Hydrotel:
    def __init__(self,
        ...
        if default_options is False and (not os.path.isfile(self.project / 'simulation' / 'simulation' / 'simulation.csv') or
                                         not os.path.isfile(self.project / 'simulation' / 'simulation' / 'output.csv')):
            raise ValueError("The simulation and output files must exist if default_options=False.")

        # Load from the files, if they exist. Otherwise use default values
        if os.path.isfile(self.project / 'simulation' / 'simulation' / 'simulation.csv') and default_options is False:
            self.simulation_options = pd.read_csv(self.project / 'simulation' / 'simulation' / 'simulation.csv',
                                                  delimiter=";", header=None, index_col=0).squeeze().to_dict()
        else:
        ...
        if os.path.isfile(self.project / 'simulation' / 'simulation' / 'output.csv') and default_options is False:
            self.output_options = pd.read_csv(self.project / 'simulation' / 'simulation' / 'output.csv', delimiter=";", header=None, index_col=0).squeeze().to_dict()
        else:
            with open(Path(__file__).parent / "hydrotel_defaults.yml") as f:
                self.output_options = yaml.safe_load(f)["output_options"]

        # Update options with the user-provided ones
        self.update(simulation_options=simulation_options, output_options=output_options)
    ...
    def update(self,
               *,
               ...
               output_options: dict = None):
        """
        Updates the simulation and output files.
        ...
        if simulation_options is not None:
            # Update options with the user-provided ones
            simulation_options = deepcopy(_fix_dates(_fix_os_paths(simulation_options))) or {}
            ...
            df = df.replace({None: ''})
            df.to_csv(self.project / 'simulation' / 'simulation' / 'simulation.csv', sep=';', header=False, columns=[0])
    ...
    def basic_checkups(self):
        ...
        weather_file = self.project / self.simulation_options.get('FICHIER STATIONS METEO', None)
    ...
    def advanced_checkups(self, xr_open_kwargs: dict = None, **kwargs):
        structure = {"dims": ["time", "stations"], "coords": ['lat', 'lon', 'x', 'y', 'z']}
        variables_and_units = {"tasmin": "degC", "tasmax": "degC", "pr": "mm"}

        ds = xr.open_dataset(self.project / self.simulation_options['FICHIER STATIONS METEO'], **(xr_open_kwargs or {}))
        health_checks(ds, structure=structure, calendar="standard", variables_and_units=variables_and_units, **kwargs)
    ...
    def run(self,
        ...
        ds = self._standardize_outputs(id_as_dim=id_as_dim, xr_open_kwargs=xr_open_kwargs)
    ...
    def _standardize_outputs(self,
        ...
        ds = ds.rename({'troncon': 'station', 'idtroncon': 'station_id', 'debit_aval': 'streamflow'})
        if id_as_dim:
    ...
    def get_streamflow(self, **kwargs):
        ...
        import shutil
        if os.path.isfile(os.path.join(self.project, 'simulation', 'simulation', 'resultat', 'debit_aval.nc')):
            os.remove(os.path.join(self.project, 'simulation', 'simulation', 'resultat', 'debit_aval.nc'))
        shutil.copy(os.path.join(self.project, 'simulation', 'simulation', 'resultat', 'debit_aval_orig.nc'),
                    os.path.join(self.project, 'simulation', 'simulation', 'resultat', 'debit_aval.nc'))

        return xr.open_dataset(self.project / 'simulation' / 'simulation' / 'resultat' / 'debit_aval.nc', **kwargs)
...
def _fix_dates(d: dict):
    ...
    for key in ['LECTURE ETAT FONTE NEIGE', 'LECTURE ETAT TEMPERATURE DU SOL', 'LECTURE ETAT BILAN VERTICAL',
        ...
        if key in d and not pd.isnull(d[key]):
            d[key] = d[key].replace(Path(d[key]).stem.split("_")[-1],
