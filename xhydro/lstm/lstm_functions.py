...
from utilsS.create_datasets import create_LSTM_dataset, clean_nans, create_LSTM_dataset_catchment_vary, create_dataset_flexible
from utilsS.LSTM_static import training_generator, define_LSTM_model, nse_loss, run_trained_model
...
def ScaleDataset(input_data_filename, dynamic_var_tags, Qsim_pos, static_var_tags, train_pct, valid_pct):
    ...
    dynamic_data = np.empty((0, ndata))  # 7 is with including Qsim as predictor and snow
...
def SplitDataset(arr_dynamic, arr_static, q_stds, watersheds_ind, train_idx, window_size, valid_idx):
    # %%
    # Training dataset
    X_train, X_train_static, X_train_q_stds, y_train = create_LSTM_dataset_catchment_vary(
    ...
    # Validation dataset
    X_valid, X_valid_static, X_valid_q_stds, y_valid = create_LSTM_dataset_catchment_vary(
        arr_dynamic=arr_dynamic,
        arr_static=arr_static,
        q_stds=q_stds,
        window_size=window_size,
    ...
    return X_train, X_train_static, X_train_q_stds, y_train, X_valid, X_valid_static, X_valid_q_stds, y_valid
...
def RunModelAfterTraining(w, arr_dynamic, arr_static, q_stds, window_size, train_idx, batch_size_val,
                          watershed_areas, filename_base, name_of_saved_model, valid_idx, test_idx, all_idx):

    # Run trained model on the training period and save outputs
    run_trained_model(arr_dynamic, arr_static, q_stds, window_size, w, train_idx, batch_size_val,
                      watershed_areas, file_ID='training', filename_base=filename_base,
                      name_of_saved_model=name_of_saved_model, postdate=False, cleanNans=True)

    # Run trained model on the validation period and save outputs
    run_trained_model(arr_dynamic, arr_static, q_stds, window_size, w, valid_idx, batch_size_val,
                      watershed_areas, file_ID='validation', filename_base=filename_base,
                      name_of_saved_model=name_of_saved_model, postdate=False, cleanNans=True)

    # Run the trained model on the testing period and save outputs
    run_trained_model(arr_dynamic, arr_static, q_stds, window_size, w, test_idx, batch_size_val,
                      watershed_areas, file_ID='testing', filename_base=filename_base,
                      name_of_saved_model=name_of_saved_model, postdate=False, cleanNans=True)
