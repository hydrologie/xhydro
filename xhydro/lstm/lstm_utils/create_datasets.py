...
def plot_boxplot_results(watersheds_ind, filename_base, filename_base_retrain):
    ...
    for w in watersheds_ind:
        kge_regional[w, 0] = np.loadtxt(
            filename_base + "_KGE_training_" + str(w) + ".txt"
        )
        kge_regional[w, 1] = np.loadtxt(
            filename_base + "_KGE_validation_" + str(w) + ".txt"
        )
        kge_regional[w, 2] = np.loadtxt(
            filename_base + "_KGE_testing_" + str(w) + ".txt"
        )
        kge_retrain[w, 0] = np.loadtxt(
            filename_base_retrain + "_KGE_training_" + str(w) + ".txt"
        )
        kge_retrain[w, 1] = np.loadtxt(
            filename_base_retrain + "_KGE_validation_" + str(w) + ".txt"
        )
        kge_retrain[w, 2] = np.loadtxt(
            filename_base_retrain + "_KGE_testing_" + str(w) + ".txt"
        )
