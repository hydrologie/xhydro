...
#         # FIXME: No need. ds.where(ds.id.str.contains("^03"), drop=True)
#         """
...
#         # FIXME: No need. xclim indexers with doy_bounds or date_bounds should be used instead
#         """
...
#         # FIXME: No need. Replace with xclim indexers
#         """
...
#         # FIXME: No need. custom QMAX indicator
#         """
...
#         # FIXME: Replaced + custom indicator.
#         # FIXME: How do you sum with a tolerance? Won't it get biased?
#         ds = self.copy()
#
...
#             return pd.to_timedelta(f"1{timestep}").total_seconds()
#
#         if dates is None:
...
def fit(
    ...
    ----------
    ds : xr.Dataset
        Dataset containing the data to fit.
    distributions : list of str, optional
        List of distribution names as defined in `scipy.stats`.
    ...
    Notes
    -----
    In order to combine the parameters of multiple distributions, the size of the `dparams` dimension is set to the maximum number of parameters of the distributions.
    """
    ...
    for v in ds.data_vars:
        ...
        for d in distributions:
            ...
            params.append(p)
        params = xr.concat(params, dim="scipy_dist")
...
# TODO: Test that this actually works, since the order of dparams is not guaranteed.
def parametric_quantiles(ds, t: Union[int, list], mode: str = "max"):
    ...
    ds : xr.Dataset
        ...
        Must have a dimension `scipy_dist` containing the distribution names.
    ...
    """
    distributions = ds["scipy_dist"].values.tolist()
...
def criteria(ds, params):
    """Compute information criteria (AIC, BIC, AICC) for fitted distributions, using the log-likelihood.
    ...
    def _get_criteria_1d(da, p, dist):
        da = np.atleast_2d(da).T
        p = np.atleast_2d(p).T

        llf = np.nansum(dist.logpdf(da, *p), axis=0)  # log-likelihood
        nobs = np.sum(np.isfinite(da), axis=0)
        df_modelwc = len(params)
        dof_eff = nobs - df_modelwc - 1.0

        aic = eval_measures.aic(llf=llf, nobs=nobs, df_modelwc=len(params))
        bic = eval_measures.bic(llf=llf, nobs=nobs, df_modelwc=len(params))
    ...
    out = []
    distributions = params["scipy_dist"].values.tolist()
    for v in ds.data_vars:
        criteria = []
        for d in distributions:
            ...
            crit.attrs.pop("method")
            crit.attrs.pop("estimator")
            crit.attrs.pop("min_years")
            criteria.append(crit)

        criteria = xr.concat(criteria, dim="scipy_dist")
        out.append(criteria)
